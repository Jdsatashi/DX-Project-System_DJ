...
def get_live_stream(instance):
    try:
        return instance.live_stream
...
def update_order_times(sender, instance, **kwargs):
    live_stream = get_live_stream(instance)
    if live_stream:
        unique_orders = LiveStreamOfferRegister.objects.filter(live_stream=live_stream, register=True).values(
            'phone').distinct().count()
        statistic, created = LiveStreamStatistic.objects.get_or_create(live_stream=live_stream)
        statistic.order_times = unique_orders
        statistic.save()
